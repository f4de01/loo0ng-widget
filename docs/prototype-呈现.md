...
2. **但它盖的是整个窗口矩形**：12px 透明外边距与圆角外的四个角都被磨砂成一块方板，柔投影也画在这块板上。要圆角就得接受角上那一小块方磨砂，要方板消失就得放弃系统模糊。**裁定（2026-09-22，看过截图后）：不接受方板，Windows 上不调 Acrylic，停在 90% 自绘那层。**见 `prototype-呈现/host-corners.png`（左：Acrylic 带外边距；中：Acrylic 去外边距；右：不开效果）。
...
5. **观感**：两页两形式加自绘卡片放在一起成立，三套色值都在「收紧不推倒」那条线上，差别在纸色冷暖与对比。**裁定：取 A 纸。**
...
- Windows 不开效果，用 90% 底，不调 `setEffects`；60% 那档只留给将来真有系统模糊、且模糊能贴着圆角形状的平台（mac 进交付票看）。
